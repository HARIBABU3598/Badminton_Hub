...
const sections = [
  ...
  { title: 'Rules and Regulations', icon: Rules },
  { title: 'Faults', icon: Faults },
  { title: 'Shots', icon: Shots },
  { title: 'Match Scorecard', icon:Score },
  { title: 'Training and Workouts', icon: Training },
  { title: 'Courts Location', icon: Login },
  { title: 'Nearby Sport Shops', icon:Shop },
  { title: 'Materials', icon:Equip },
];
...
const Dashboard = () => {
  ...
  <div
    key={index}
    className="bg-white text-black rounded-lg px-4 py-4 shadow-lg hover:bg-white/30 hover:text-white transition border "
  >
    <div className="flex items-center space-x-3 mb-2">
      <h2 className="text-2xl font-bold text-center">{title}</h2>
    </div>
    <div className="w-full h-[2px] bg-black"></div>
     <img src={icon} alt={title} className="mt-[2%] w-200 h-80 rounded" />
  </div>
))}
